...
Each file type is matched by its filename suffix (e.g. `.service.ts`, `.guard.ts`, `.dto.ts`). Plural variants — such as `.guards.ts` or `.strategies.ts` — are also recognised for types that commonly appear in that form.
